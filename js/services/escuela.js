...
const consultar_escuelas = async (request, response) => {
  ...
  const query = await pool_query(
    `SELECT escuela.id_escuela,
                   ...
                   inmueble.longitud
            FROM escuela
                     LEFT JOIN turno1 on escuela.turno1_id_turno1 = turno1.id_turno1
                     LEFT JOIN status on escuela.status_id_status = status.id_status
...
const consultar_escuela = async (request, response) => {
  ...
  const escuelas = `select escuela.id_cct                                        as CCT,
             ...
             inmueble.latitud,
             inmueble.longitud,
             inmueble.id_asenta_codigo_postal,
...
const editar_escuela = async (request, response, socket) => {
  ...
  if ((request.session.rol_id === 1) || (token_acceso === TOKEN_WEB)) {
    const query_inmueble_director = await pool_query_unique(`SELECT inmueble_id_inmueble, director_id_director FROM escuela Where id_escuela = '${id_escuela}';`)
    const {response: {inmueble_id_inmueble, director_id_director}} = query_inmueble_director

    //Editar director
    const llaves_filtrar_director = ["nombre_director"]
    const where_director = {id_director: director_id_director}
    const query_director = await pool_query(pool_query_update(await filtrar_llaves(request.body, llaves_filtrar_director), where_director, "director"), "", "")

    //Editar inmueble
    const llaves_filtrar_inmueble = ["municipio_id_municipio", "id_asenta_codigo_postal", "asenta2", "vialidad_principal", "vialidad_derecha", "vialidad_izquierda", "vialidad_posterior", "num_ext", "alf_ext", "num_int", "alf_int", "desc_ubicacion", "latitud", "longitud"]
    const where_inmueble = {id_inmueble: inmueble_id_inmueble}
    ...
    if (query_escuela.success && query_director.success && query_inmueble.success) {
      const query_socket = await pool_query_unique(
        `select escuela.id_cct                                        as CCT,
             escuela.id_escuela,
             escuela.problematica,
      ...
      return response.status(200).json(query_escuela);
    } else {
      return response.status(400).json(query_escuela);
    }
  } else {
    return response.status(200).json(message_failure("No tienes los permisos para esta acción"));
  }
