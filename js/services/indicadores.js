...
const consultar_indicadores_internacionales = async (request, response) => {
  ...
  if (request.session.rol_id === 1 || token_acceso === TOKEN_WEB) {
    ...
    const query_indicador = await pool_query(`SELECT indicador_mexico, nacional_porcentaje, hidalgo_porcentaje, posicion, meta_id FROM indicador;`, "", "Error, no se pudieron consultar indicadores internacionales");
